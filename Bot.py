...
os.chdir('D:/Bot Clients/NoLifeSavings')
dbdir = "D:/Bot Clients/NoLifeSavings/db.json"
...
@client.event
async def on_ready():
    print('NoLifeSavings now online!')
    ...
    with open(dbdir, "r+") as f:
        ...
        if member:
            if str(member.id) in data:
                ...
                embed.set_thumbnail(url="https://i.ibb.co/NWfYRSz/Logo.png")
    ...
    with open(dbdir, "r+") as f:
        ...
        if member:
            if amount:
                ...
                if data[str(ctx.author.id)]["money"] >= amount:
                    data[str(ctx.author.id)]["money"] = data[str(ctx.author.id)]["money"] - amount
                    data[str(member.id)]["money"] = data[str(member.id)]["money"] + amount
                    f.seek(0)
                    f.truncate()
                    json.dump(data, f, indent=4)
                else:
                    await ctx.reply("You can't afford to transfer that")
            else:
    ...
    with open(dbdir, "r+") as f:
        ...
        if str(ctx.author.id) in data:
            ...
            if money < 0:
                ...
                await ctx.reply(f"You got robbed of ${money}")
            elif money == 0:
                await ctx.reply(f"You got scammed and they didn't pay you")
            else:
                            ...
                            if int(choice) < 0 or int(choice) > 36:
                                ...
                                if number == 0:
                                    ...
                                    if int(choice) == number:
                                        ...
                                        f.truncate()
                                        json.dump(data, f, indent=4)
                                        await ctx.reply(f"The ball landed on {number} {colour}, you won ${amount*35}")
                                    else:
                                        await ctx.reply(f"The ball landed on {number} {colour}, you lost")
                        else:
                            choice = choice.lower()
                                ...
                                if number == 0:
                                    ...
                                    if choice in ["Red", "Black"]:
                                        if choice == colour:
                                            ...
                                            f.truncate()
                                            json.dump(data, f, indent=4)
                                            await ctx.reply(f"The ball landed on {number} {colour}, you won ${amount*2}")
                                        else:
                                            await ctx.reply(f"The ball landed on {number} {colour}, you lost")
                                    else:
                                        if (number % 2) == 0:
...
async def shop(ctx):
    ...
    embed.set_thumbnail(url="https://i.ibb.co/NWfYRSz/Logo.png")
    embed.add_field(name="Example Item", value="Price\nDescription")
    await ctx.reply(embed=embed)
...
async def help(ctx):
    ...
    embed.add_field(name="Slut", value="Questionable\n`!slut`\n2 Minute Cooldown")
...
#Run Token
client.run('OTU3MzY4NzA3OTA1MTE4Mjc4.Yj9xFw.T2c6wpXz5lOmp_aO7HkCmRoyuQc')